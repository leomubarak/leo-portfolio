...
/** Direct routes to Leo, built only from values that are actually filled in. */
...
export function Contact() {
  const { values, errors, status, statusMessage, setValue, handleBlur, setHoneypot, submit } =
    useContactForm();
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    // Send focus to the first problem rather than leaving it to be hunted for.
    if (firstInvalid) {
  ...
  return (
    <Section id="contact">
      <SectionHeading
        ...
        description="Have a project idea, a programming question, a collaboration in mind, or just want to connect? I'd love to hear from you."
      />
          ...
          <Card padding="lg" className="h-full">
            ...
            {methods.length > 0 ? (
              ...
              <p className="mt-5 text-sm leading-relaxed text-content-muted">
                {import.meta.env.DEV
                  ? 'Add `email` and `whatsappNumber` to src/data/site.ts and they will be listed here.'
                  : 'Use the form and I will reply by email.'}
              </p>
            )}
                  ...
                  {activeSocialLinks.map((link) => (
                    <li key={link.platform}>
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2.5 text-sm text-content-muted transition-colors hover:text-content"
                      >
            ...
            <form ref={formRef} onSubmit={handleSubmit} noValidate>
              ...
              <div role="status" aria-live="polite" className="mt-5 empty:mt-0">
                {statusMessage && (
                  <p
                    className={
                      status === 'success'
                        ? 'flex items-start gap-2 rounded-xl border border-accent bg-accent-soft px-4 py-3 text-sm text-content'
                        : 'flex items-start gap-2 rounded-xl border border-line-strong bg-surface-sunken px-4 py-3 text-sm text-content-muted'
                    }
                  >
                    {status === 'success' ? (
                      <CheckCircle2
              ...
              {import.meta.env.DEV && !isContactEndpointConfigured && (
                <p className="mt-5 rounded-xl border border-dashed border-line-strong px-4 py-3 font-mono text-[11px] leading-relaxed text-content-subtle">
                  No VITE_CONTACT_ENDPOINT set — submissions will not be delivered. See the README
                  for connecting Formspree, Web3Forms, EmailJS or your own endpoint.
                </p>
              )}
            </form>
          </Card>
