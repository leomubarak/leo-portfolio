...
const levels = Object.entries(skillLevelMeta)
  ...
  .map(([level, meta]) => ({ level: level as SkillLevel, meaning: meta.meaning }));
  ...
  return (
    <Section id="skills" tone="sunken">
      <SectionHeading
        ...
        description="Grouped by what I use them for, and labelled by how well I actually know them."
      />

      <div className="mt-14 grid gap-6 lg:grid-cols-[16rem_1fr] lg:gap-10">
        <div role="tablist" aria-label="Skill categories" className="-mx-5 flex gap-2 overflow-x-auto px-5 pb-2 lg:mx-0 lg:flex-col lg:overflow-visible lg:px-0 lg:pb-0">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  selected ? 'border-accent bg-accent text-accent-contrast' : 'border-line bg-surface-raised text-content-muted hover:border-line-strong hover:text-content',
                )}
              >
                <Icon size={16} aria-hidden="true" />
                <span className="font-medium">{category.label}</span>
                <span className={cn('ml-auto hidden font-mono text-[11px] lg:inline', selected ? 'text-accent-contrast/70' : 'text-content-subtle')}>
        ...
        <div role="tabpanel" id={`skills-panel-${active.id}`} aria-labelledby={`skills-tab-${active.id}`} tabIndex={0} className="rounded-card focus-ring">
          <AnimatePresence mode="wait">
            ...
            >
              <p className="max-w-2xl text-sm leading-relaxed text-content-muted">{active.description}</p>
                    ...
                    <Card padding="sm" className="h-full">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          {skill.note && <p className="mt-1 text-xs leading-relaxed text-content-subtle">{skill.note}</p>}
                        </div>
                      </div>
        ...
        <div className="mt-10 rounded-card border border-line bg-surface-raised p-6 sm:p-7">
          ...
          <dl className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {levels.map(({ level, meaning }) => (
              <div key={level}>
                <dt><SkillLevelMeter level={level} /></dt>
