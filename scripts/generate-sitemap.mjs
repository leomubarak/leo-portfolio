 ...
 *
 * Project slugs are read straight out of src/data/projects.ts, so adding a
 * project to that file is all it takes to get it into the sitemap.
 *
 * The site URL comes from VITE_SITE_URL (a .env file locally, or Vercel's
 * environment variables). Without it the script writes a sitemap-less
 * robots.txt and stops, rather than publishing a sitemap full of wrong URLs.
 */
...
writeFileSync(
  resolve(root, 'public/robots.txt'),
  ['User-agent: *', 'Allow: /', '', 'Disallow: /styleguide', '', `Sitemap: ${siteUrl}/sitemap.xml`, ''].join('\n'),
);
